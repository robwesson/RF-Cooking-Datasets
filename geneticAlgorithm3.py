...
# def gradientDescent(Modes, numSteps, deleteZeros):

# v2 incorporates self termination after n steps of zero improvement.

def geneticAlgorithm3(Modes, numSpecies, numMutations, deleteZeros, heuristic):
    ...
    startWeights = [10 for i in range(0,len(Modes))]
    speciesList =[]
    for i in range (0, numSpecies):
        print('--------------------------')
        speciesList.append(gradientDescent(Modes, startWeights, 200, False, heuristic))
    ...
    graphAveragePAR = []
    try:
        while countFails<100:
            childSpeciesList = copy.deepcopy(speciesList)
            
            swapVector = [random.randint(0,1) for i in range (0,numWeights)]
